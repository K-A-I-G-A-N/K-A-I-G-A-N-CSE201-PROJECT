...
public class FilterGUI extends JFrame {
 ...
 // Test-only method to allow injecting custom data into FilterGUI
    public void setAllAppsForTest(List<App> testApps) {
        this.allApps = testApps;
    }
 // Test-friendly backend filtering method
    public List<App> filterAppsByCategory(String category) {
        List<App> result = new ArrayList<>();

        for (App app : allApps) {
            if (category == null || category.isEmpty()) {
                result.add(app);
            } else if (app.genre != null &&
                       app.genre.trim().equalsIgnoreCase(category.trim())) {
                result.add(app);
            }
        }
